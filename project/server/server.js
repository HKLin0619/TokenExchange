...
app.post("/tokenminting", async (req, res) => {
  ...
  const deployedContract = await tokenContract
    ...
    .send(
      {
        from: "0x12eeB20Bf12781cAd3B9420b87153C73cBfEE9d5",
        gas: 3000000,
...
app.get("/viewtoken", async (req, res) => {
  try {
    ...
    const account =
      req.query.account || "0x12eeB20Bf12781cAd3B9420b87153C73cBfEE9d5";
...
app.post("/purchasetoken", async (req, res) => {
  ...
  try {
    ...
    const transactionReceipt = await contractInstance.methods
      ...
      .send({
        from: "0x170FC77777E7539C06f90030D4F3d6979eb9C512", //
        gas: 3000000,
        gasPrice: 20000000000,
    ...
    const buyerAddress = "0x170FC77777E7539C06f90030D4F3d6979eb9C512"; // Replace with the actual buyer's address
    await database.query(
...
app.post("/writeData", async (req, res) => {
  ...
  const buyerAddress = "0x170FC77777E7539C06f90030D4F3d6979eb9C512";
  ...
  try {
    ...
    try {
      const transactionReceipt = await contractInstance.methods
        ...
        .send({
          from: "0x170FC77777E7539C06f90030D4F3d6979eb9C512", //buyer address
          gas: 3000000,
          gasPrice: 20000000000,
        });
...
app.post("/updateData", async (req, res) => {
  try {
    const tokenName = req.body.tokenName;
    const awardId = req.body.awardid;
    const financerId = req.body.financerid;
    const funded_int = req.body.funded_int;

    await database.query(
      'UPDATE "award" SET "financerid" = $1 , "funded_int" = $2 WHERE "awardid" = $3;',
      [financerId, funded_int, awardId]
    );
    res.json({ success: true });
  } catch (error) {
